...
import com.jimplush.goose.images.ImageSaver;
...
public class ContentExtractor {
  ...
  public Article performExtraction(String urlToCrawl, String rawHtml) {
    ...
    try {
      ...
      ArrayList<String> additImages= new ArrayList<String>();
      
      //before cleaning document, grab css images
      if(config.getCss()) {
	      Pattern p = Pattern.compile("url\\(\\s*(['\"]*)(.*?)\\1\\s*\\)");
	      Matcher m = p.matcher(doc.toString());
	      int i=0;
	      while (m.find() && i++<15) {
	    	  Pattern pp = Pattern.compile("http:[/\\w\\.-]+");
	    	  Matcher mm = pp.matcher(m.group());
	    	  mm.find();
	          additImages.add(mm.group());
	      }
      }
      ...
      //  article.setMovies(extractVideos(article.getTopNode()));
        ...
        if (config.isEnableImageFetching()) {
          ...
          article.setTopImage(imageExtractor.getBestImage(doc, article.getTopNode(), additImages));
		...
		*/
        ...
        //put a title or big headline in the article's text, as a substitute for a picture
        Element headline=null;
    	Elements heads=null;
    	Integer tagIndex;
    	String headlineText="";
    	for(tagIndex=1; tagIndex<=6; tagIndex++)
    	{
    		heads=doc.getElementsByTag("h"+tagIndex.toString());
    		if(!heads.isEmpty())
    		{
    			headline = heads.first();
    			headlineText = headline.text();
    			break;
    		}
    	}        
    	//headline not found, take the title instead
        if(string.isNullOrEmpty(headlineText))
        {
        	headlineText=article.getTitle();
        }
        //title not found, just use body's text (0% to fail at this moment)
        if(string.isNullOrEmpty(headlineText))
        	headlineText = doc.body().text();
        
        article.setCleanedArticleText(headlineText);
        ...
        releaseResources(article.getTopImage().getImagePath());
      
        //save image if can and need to
        if( config.getSaveImage())
        {
        	{
        		//create an html page with site's headline instead
        		String Page = "<html><head>";
        		String title = article.getTitle();
        		if(string.isNullOrEmpty(title))
        			title = "Automated depictional webpage";
        		Page += "<title>"+title+"</title></head><body>";
        		
        		if(headline!=null)
        			Page += headline.toString();
        		else
        			Page += "<h1>"+headlineText+"</h1>";
        		
        		Page += "</body></html>";
        		
        		//Save page
        		FileWriter fw = new FileWriter(config.getLocalStoragePath()+"/page.html");
        		fw.write(Page);
        		fw.close();
...
/*    Not sure what this is for
  ...
  private Element calculateBestNodeBasedOnClustering(Document doc) {
	  return doc.getElementsByTag("html").get(0); //return whole site instead of an article
...
*/
   ...
   */
  public void releaseResources(String bestImagePath) {
    if (logger.isDebugEnabled()) {
    ...
    if (children == null) {
      ...
      for (int i = 0; i < children.length; i++) {
        ...
        if (filename.startsWith(this.linkhash) && 
        	(!config.getSaveImage() || bestImagePath.indexOf(filename)==-1)) {
