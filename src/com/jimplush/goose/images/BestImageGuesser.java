...
public class BestImageGuesser implements ImageExtractor {
  ...
  public Image getBestImage(Document doc, Element topNode, ArrayList<String> additImages) {
    this.doc = doc;
    ...
    if (image.getImageSrc() == null) {
      this.checkForLargeImages(topNode, 0, 0, additImages);
    }

    // fall back to meta tags, these can sometimes be inconsistent which is why we favor them less
    if (config.getAllImages() && image.getImageSrc() == null) {
   ...
   * @param node
   */
  private void checkForLargeImages(Element node, int parentDepth, int siblingDepth, ArrayList<String> addImgs) {
    if (node == null) return;

    Elements images = node.select("img");
    
    //handle css images first
    if(parentDepth==0 && siblingDepth==0 )
	    for (int i=0; i<addImgs.size(); i++)
	    {
	    	Element img = node.clone();
	    	img.attr("src", addImgs.get(i));
	    	//add nodes in the beginning at order of arrival
	    	images.add(i,img);
	    }
    ...
    if (highScoreImage != null) {
    	//Idan's addition
    	this.image.setImagePath(highScoreImage.attr("tempImagePath"));
      ...
      if (parentDepth < 2) {
        ...
        if (prevSibling != null) {
          ...
          siblingDepth++;
          this.checkForLargeImages(prevSibling, parentDepth, siblingDepth, addImgs);
        } else {
          if (logger.isDebugEnabled()) {
            logger.debug("no more sibling nodes found, time to roll up to parent node");
          }
          parentDepth++;
          this.checkForLargeImages(node.parent(), parentDepth, siblingDepth, addImgs);
        }
  ...
  private ArrayList<Element> findImagesThatPassByteSizeTest(ArrayList<Element> images) {
    ...
    for (Element image : images) {
      if (cnt > 15) {
        if (logger.isDebugEnabled()) {
          logger.debug("Abort! they have over 15 images near the top node: " + this.doc.baseUri());
        }
        return goodImages;
      }
  ...
  private HashMap<Element, Float> downloadImagesAndGetResults(ArrayList<Element> images, int depthLevel) {
    ...
    for (Element image : images) {
      ...
      if (cnt > 15) {
        if (logger.isDebugEnabled()) {
          logger.debug("over 15 images attempted, that's enough for now");
        }
        break;
      }
      ...
      try {
        ...
        logger.error(e.toString());
        //continue;
        throw(new Error("image magick error"));
      }
